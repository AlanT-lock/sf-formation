...
export async function POST(request: NextRequest) {
  ...
  if (formateurError) {
    ...
    return NextResponse.json({ error: formateurError.message }, { status: 500 });
  }

  return NextResponse.json({
    ...formateur,
    username,
    message: "Formateur créé. Identifiant : " + username + " (le formateur définira son mot de passe à la première connexion).",
  });
}
